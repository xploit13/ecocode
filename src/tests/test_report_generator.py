...
def test_generate_text_report(report_generator, tmpdir):
    sample_data = {"Total Energy Usage": "120 J", "Execution Time": "15 seconds"}
    report_path = report_generator.generate_text_report(sample_data, filename="text_report.txt")
...
def test_generate_json_report(report_generator, tmpdir):
    sample_data = {"Total Energy Usage": "120 J", "Execution Time": "15 seconds"}
    report_path = report_generator.generate_json_report(sample_data, filename="json_report.json")
...
def test_generate_html_report(report_generator, tmpdir):
    sample_data = {"Total Energy Usage": "120 J", "Execution Time": "15 seconds"}
    report_path = report_generator.generate_html_report(sample_data, filename="html_report.html")
...
def test_empty_data_handling(report_generator):
    report_path = report_generator.generate_text_report({}, filename="empty_report.txt")
    assert os.path.exists(report_path), "Report file should be created even for empty data."
    with open(report_path, "r") as file:
        content = file.read()
